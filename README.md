...
- **6 种自动布局**：力导向（默认）/ 环形 / 层级树状 / 网格 / **分簇**（按分组聚簇）/ **放射状**（核心人物为中心）
- **动态画布**：拖拽、缩放、搜索定位、批量框选、右键菜单、撤销重做；**单击人物常驻显示其关联**（再点/ESC 恢复）
- **9 套主题**：浅色 / 深色 / 简约 / 商务 / 森语绿 / 罗兰紫 / 深海蓝 / 落日橙 / 樱绯粉
...
- **力导向**（默认）：节点排斥 + 关系强度自适应拉伸，大数据采用网格近似算法（Barnes-Hut 简化），3000 节点约 4 秒完成
- **环形**：按分组/姓名排序上环，每环容量按周长自适应
- **层级树状**：BFS 分层 + 叶子宽度均摊，宽高双自适应（深链不爆栈）
- **网格 / 分簇 / 放射状**：均支持节点锁定参与/不参与布局
...
- **9 套全局主题**：浅色 / 深色 / 简约 / 商务 / 森语绿 / 罗兰紫 / 深海蓝 / 落日橙 / 樱绯粉（画布 + 面板 UI 同步）
...
| 功能 | 说明 |
|---|---|
| 图片导出 | PNG / JPG / 透明底 PNG / PDF，1×–3× 分辨率（面积预算自动降档并提示） |
...
| 滚轮 / Ctrl+滚轮 | 画布缩放 |
| ESC | 关闭弹窗 → 退出全屏 → 取消固定聚焦 → 取消选中 → 退出溯源 → 清空筛选（逐级） |
...
│   ├── layout.js           # 六种布局算法（力导向/环形/树状/网格/分簇/放射）
...
- 唯一第三方库为本地引入的 SheetJS（Excel 读写），其余全部自研实现（含 Barnes-Hut 式网格近似力导向布局、
  二次贝塞尔曲线布线、极简 PDF 生成器、AES-GCM 工程加密等）
- 数据完全本地：IndexedDB 工程库 + localStorage 设置/会话恢复，无任何网络请求
- 安全加固：CSV 公式注入防护（=+-@ 前缀）、导入文件白名单与大小限制、工程/JSON 字段白名单校验（防原型污染与非法值导致渲染崩溃）、加密工程使用 PBKDF2(12 万次)+AES-GCM 随机盐/IV
