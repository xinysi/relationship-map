...
const DataIO = {
  ...
  MD_TEMPLATE: "# 《示例故事》剧情整理模板\n\n> 说明（导入前可删除本段）：本模板演示剧情文档的四类写法，可混合使用。\n> ① 主要角色列表（`- 姓名(英文)——简介`）\n> ② 家族树（代码块内 `├─ └─` 与 `→` 子女链）\n> ③ 关系恩怨（`A × B` 夫妻 / `A ↔ B` 其他关系）\n> ④ 剧情梗概与章节小节（自动生成事件）；叙述句也可直接写关系（`A与B是恋人` / `A杀死了B`）\n> 关系强度无需填写：解析器按类型自动分级，正文强语气词（宿敌/至爱/决裂）会再 +1。\n\n## 主要角色\n\n- 主角（Hero）——故事主角，示例简介。\n- 安娜·格雷（Anna Gray）——格雷家次女，示例简介。\n- 盖尔·史密斯——示例人物。\n- 杰迪（Jedi）——示例：与赫尔曼是兄弟。\n\n## 家族树\n\n```\n格雷（祖父）\n ├─ 安娜·格雷\n └─ 路易莎·格雷 → 罗伯特·格雷\n```\n\n## 关系恩怨\n\n- 安娜·格雷 × 盖尔·史密斯（示例夫妻）\n- 杰迪 ↔ 赫尔曼（示例，兄弟反目）\n\n## 剧情梗概\n\n　　故事一句话概括，自动生成「剧情梗概」事件。\n\n## 详细剧情\n\n### 第一章 相遇\n\n安娜与盖尔是恋人；他们一同调查真相。\n\n### 第二章 对峙\n\n杰迪与赫尔曼本是挚友；杰迪杀死了赫尔曼。\n\n## 时间线（可选）\n\n> 章节小节（### 第X章 …）会自动生成时间线事件，无需单独填写表格。",
  ...
  downloadTemplate(kind) {
    ...
    if (kind === 'xlsx') {
      ...
      Utils.download('人物关系网-Markdown剧情模板.md', this.MD_TEMPLATE, 'text/markdown');
    } else {
      const tpl = {
