...
> 首次打开会有新手引导；点击「加载示例数据体验」可立即查看《长夜档案与镜城来信》全系列示例（示例数据由项目示例文件生成，随仓库分发）。
...
- **7 种自动布局**：力导向（默认）/ 环形 / 层级树状 / 网格 / **分簇**（按分组聚簇）/ **自动分簇**（Louvain 社区发现自动识别圈子，无需手填分组）/ **放射状**（核心人物为中心）；**社区折叠视图**（≥20 人）：整图折叠为圈子超节点（显示名/人数），点击查看成员并聚焦
...
- **🤖 AI 智能提取（可选）**：任意小说/剧本文本 → OpenAI 兼容 LLM 自动抽取人物/关系/事件（**服务预设一键填入**：DeepSeek/OpenAI/智谱/通义/Kimi；提取结果**逐条勾选确认**后应用，密钥 AES-GCM 加密后仅存本机，明文不落盘）
<img src="assets/acc64eb88126a6c4fd63f4cde6a52694.png" width="640" height="638" alt="acc64eb88126a6c4fd63f4cde6a52694.png">

- **一键只读分享页**：单文件 HTML（含缩放/悬浮/点击聚焦/时间轴交互，**界面语言跟随中/英**），直接发送他人即可查看
...
- **80 套全局主题**：经典 / 自然 / 暖阳 / 冷调 / 粉紫 / 炽金 / 复古 / 潮流 / 国风 / 甜品 / 科幻 / 暗黑 / 金属 / 暖纱 / 织锦 / 灯会 16 类风格（画布 + 面板 UI 同步），含复古撞色、甜酷、中式、马卡龙、千禧年代、赛博、雪花电视、金属、烟花等混搭风格；**自定义主题可导出/导入 JSON 分享**
...
内置标准模板可在「数据 → 下载模板」中获取（Excel / CSV / JSON / **Markdown 剧情文档** 四种，前三种含三张表单；Markdown 模板展示全部支持格式，模板本身可直接导入验证）。仓库内亦提供模板文件本体：`sample/Markdown剧情模板.md`、`sample/标准导入模板.json`（与下载内容一致；网页「下载模板」优先读取它，改文件即改模板，拿不到才回退内置版本）。字段规范：
...
面向剧情梳理场景，支持将整份剧情 Markdown 文档一键转换为关系网 + 时间线（标准模板见「数据 → 下载模板」，仓库内模板文件本体见 `sample/`；`sl/` 下的全量正文与大型原始文档为本地私有、不随代码分发）：
...
- **叙述句式识别**：`A与B是恋人`、`A杀死了B`（60+ 动词映射类型）、方位句（`A:…B之妻/之妹/之母`）、被动式（`被 X 教唆/绑架`）、长句内嵌（`尽管莫琳与埃米特本是挚友`）等自然语句自动建关系，带简称匹配与歧义防护
...
- **模板书写提示**（避免"未结构化"告警）：每个角色只在一行出现、角色括号内元信息用分号分隔、纯叙述句归入「详细剧情」或写成关系句（`A与B是…`）；模板本体（「数据 → 下载模板」）可直接导入验证
...
├── tools/                  # 辅助工具（md-import-check.js：Markdown 导入前自查，node tools/md-import-check.js 文件.md，输出人物/关系/时间线统计与未结构化/格式问题定位）
├── sample/                 # 标准导入模板示例（人物信息表.csv / 关系信息表.csv / 时间线事件表.csv / Markdown剧情模板.md）
...
- 安全加固：CSV 公式注入防护（=+-@ 前缀）、导入文件白名单与大小限制、工程/JSON 字段白名单校验（防原型污染与非法值导致渲染崩溃）、加密工程使用 PBKDF2(12 万次)+AES-GCM 随机盐/IV、分享页与 SVG 导出的文本转义（防 script 注入）、LLM 回复 JSON 容错解析（仅接受含 schema 键的对象，拒绝截断碎片）、API 密钥本地加密存储（AES-GCM 256 + PBKDF2 12 万次派生 + 随机盐/IV，明文不落盘，旧版明文密钥首次加载自动加密迁移，非安全上下文降级为明文并提示）
...
- **测试**：`npm test`（Node 22+ 内置 test runner，零依赖，**63 项**）。覆盖：数据模型（净化/CRUD/撤销栈/固定聚焦）、导入导出（CSV 公式注入防护、Markdown 解析与强度推断、JSON/CSV 导入、info 级别分离、SVG 文档与转义、追加同名合并）、7 种布局（坐标有效性 / 深链防爆栈 / 力导向收敛）、社区发现（双社区/子社区/无关系/性能）、图分析（最短路径/度中心性/Brandes 介数）、分享页（结构/注入防护）、LLM（回复解析容错、配置校验、ID 唯一、空返回重试）
